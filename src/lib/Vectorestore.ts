 ...
 *
 * Isko app startup par call karo (App.tsx ya main layout mein)
 * taaki pehli user query par delay na ho.
 *
 * Idempotent hai — baar baar call karne se koi fark nahi padta.
 */
 ...
 *
 * Process:
 *   1. Query ko embed karo (same model se)
 *   2. Cached knowledge embeddings se cosine similarity compute karo
 *   3. Optional: Deity filter lagao
 ...
 * formatContextForLLM()
 *
 * Retrieved chunks ko Claude ke system prompt mein inject karne
 * ke liye ek readable string mein convert karo.
 *
 * @param results - retrieveRelevantChunks() ka output
...
/**
 * _doInit()
 * Actual initialization — model load + all chunks embed.
 * Sirf ek baar run hoga (singleton pattern se guard kiya hai).
 */
async function _doInit(): Promise<void> {
  try {
    console.log("[Vectorstore] Initializing embedding model...");

    // Step 1: Embedding pipeline load karo
    // "feature-extraction" task — text → vector
    _embedder = await pipeline("feature-extraction", MODEL_NAME);
    console.log("[Vectorstore] Model loaded ✅");

    // Step 2: Saare knowledge chunks embed karo (ek baar, phir cache)
    console.log(`[Vectorstore] Embedding ${ALL_CHUNKS.length} knowledge chunks...`);

    const embeddings: number[][] = [];

    // Ek ek karke embed karo (batch embedding bhi possible hai lekin yeh safer hai)
    ...
    // Reset karo taaki retry possible ho
    _embedder = null;
 ...
 * cosineSimilarity()
 *
 * Do vectors ke beech cosine similarity compute karo.
 * Range: -1 (opposite) to 1 (identical) — typically 0 to 1 for text.
 *
 * Formula: (A · B) / (|A| × |B|)
 * Note: Normalized vectors ke liye sirf dot product kaafi hai (|A|=|B|=1)
 * Lekin normalization error ke liye safeguard rakha hai.
 */
